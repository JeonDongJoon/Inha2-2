...
<body>
	<%
		...
		//해당 'profileID'에 대한 게시글을 가져온 다음 세션을 통하여 작성자 본인이 맞는지 체크한다
		...
		if(!userID.equals(profileDTO.getUserID())){
			...
			if(request.getParameter("profileTitle") == null || request.getParameter("profileContent") == null
		|| request.getParameter("profileTitle").equals("") || request.getParameter("profileContent").equals("")){
		PrintWriter script = response.getWriter();
		script.println("<script>");
		...
		int result = profileDAO.update(profileID, request.getParameter("profileTitle"), request.getParameter("profileContent"));
		// 데이터베이스 오류인 경우
		if(result == -1){
			PrintWriter script = response.getWriter();
			script.println("<script>");
